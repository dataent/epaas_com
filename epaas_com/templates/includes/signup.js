setup_signup = function(page) {
	...
	function set_availability_status(is_available, subdomain, validation_msg) {
		...
		if(is_available) {
			...
			page.find('.availability-status span').html(`${subdomain}.epaas.xyz is available!`);
		} else {
			// not available state
			...
			page.find('.availability-status span').html(`${subdomain}.epaas.xyz is already taken.`);
		}
	}
	...
	$('[name="subdomain"]').on("keyup", function() {
		$('.subdomain-help').text($(this).val() || window.epaas_signup.subdomain_placeholder);
	});
	...
	function check_if_available(subdomain, callback) {
		setTimeout(function() {
			dataent.call({
				method: 'epaas_com.api.check_subdomain_availability',
...
// 	}
	...
	if (query_params.domain) {
		...
		if (subdomain.indexOf(".epaas.xyz")) {
			subdomain = subdomain.replace(".epaas.xyz", "");
		}
